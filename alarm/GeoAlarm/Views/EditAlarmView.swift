    ...
    var body: some View {
        Form {
            Section("アラーム情報") {
                ...
                DatePicker("日時", selection: $time, displayedComponents: [.date, .hourAndMinute])
                Toggle("有効", isOn: Binding(
                    get: { enabled },
                    set: { newValue in
                        // 範囲チェックを実行（元の目的地に対して）
                        guard let withinRange = locationService.isWithinRange(of: alarm.coordinate) else {
                            toggleErrorMessage = "現在地を取得できません。位置情報を許可してください。"
                            return
                        }
                        guard withinRange else {
                            toggleErrorMessage = "この場所に到着していないため、アラームの有効/無効を変更できません。"
                            return
                        }
                        // 範囲内の場合のみ有効/無効を切り替え
                        enabled = newValue
                ...
                if let message = toggleErrorMessage {
                    Text(message)
                        .font(.footnote)
                        .foregroundStyle(.red)
                }
        ...
        .alert("位置情報が必要です", isPresented: Binding(get: { toggleErrorMessage != nil }, set: { _ in toggleErrorMessage = nil })) {
